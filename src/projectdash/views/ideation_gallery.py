...
class IdeationGalleryView(Static):
    ...
    def compose(self) -> ComposeResult:
        with Horizontal(id="ideation-layout"):
            with Vertical(id="ideation-main"):
                ...
                with Horizontal(id="ideation-hero"):
                    with Vertical(id="ideation-hero-chart"):
                        yield Static("FOCUS CHART", id="ideation-hero-title")
                        yield Vertical(id="ideation-hero-body")
                    with Vertical(id="ideation-hero-kpis"):
                        yield Static("SIGNAL", id="ideation-hero-kpis-title")
                        yield Static("", id="ideation-hero-kpis-body")
    ...
    def _mount_plot_cards(self, container: Vertical, cards: list[IdeationCard], start_index: int) -> None:
        for index, card in enumerate(cards, start=start_index):
            ...
            if selected:
                card_box.add_class("is-selected")
            ...
            card_box.mount(Static(self._card_kpi_text(card), classes="ideation-card-kpis"))
            card_box.mount(Static(f"cue: {self._wrap(card.action_hint, 38)}", classes="ideation-card-cue"))
            container.mount(card_box)

    def _draw_plotext_line(
        self,
        widget,
        spec: LineChartSpec,
        *,
        plot_size: tuple[int, int] | None = None,
        x_frequency: int | None = None,
        y_frequency: int | None = None,
    ) -> None:
        plt = widget.plt
        ...
        if plot_size:
            ...
            plot_width = 44
        self._safe_call(plt, ["plotsize", "plot_size"], plot_width, plot_height)
        ...
        self._safe_call(plt, ["xfrequency"], x_frequency or max(1, min(6, len(tick_positions))))
        self._safe_call(plt, ["yfrequency"], y_frequency or 4)
    ...
    def _card_block_text(self, card: IdeationCard, index: int) -> Text:
        ...
        text.append(f"   {self._card_kpi_text(card)}\n", style="#8a8a8a")
    ...
    def _refresh_hero(self, cards: list[IdeationCard]) -> None:
        hero_body = self.query_one("#ideation-hero-body", Vertical)
        hero_body.remove_children()
        hero_kpis = self.query_one("#ideation-hero-kpis-body", Static)

        selected = self._selected_card()
        if selected is None:
            hero_body.mount(Static("Select a concept to focus the chart.", classes="placeholder-text"))
            hero_kpis.update("No selection.")
            return

        spec = self._line_spec(selected)
        if spec is not None:
            if self._plotext_enabled():
                plot_widget = PlotextPlot(classes="ideation-hero-plot")
                hero_body.mount(plot_widget)
                self._draw_plotext_line(plot_widget, spec, plot_size=(72, 18), x_frequency=6, y_frequency=6)
            else:
                hero_body.mount(
                    Static(
                        self._line_renderer.render_hires(
                            ...
                            selected_series_index=self._line_selected_series,
                            window_start=self._line_window_start,
                            window_size=max(8, min(len(spec.x_labels), 16)),
                            cell_rows=4,
                        ),
                        classes="ideation-hero-fallback",
                    )
                )
            hero_kpis.update(self._hero_kpi_text(spec))
            return

        hero_body.mount(Static(self._non_line_preview(selected.chart_kind), classes="ideation-hero-fallback"))
        hero_kpis.update(f"{selected.title}\npreview-only concept")

    def _hero_kpi_text(self, spec: LineChartSpec) -> str:
        series_index = self._line_selected_series % len(spec.series)
        focus = spec.series[series_index]
        summary = self._line_kpi(spec, series_index)
        return f"{focus.name}\n{summary}"

    def _card_kpi_text(self, card: IdeationCard) -> str:
        spec = self._line_spec(card)
        if spec is None:
            return "preview-only"
        series_index = 0
        return self._line_kpi(spec, series_index)

    def _line_kpi(self, spec: LineChartSpec, series_index: int) -> str:
        if not spec.series:
            return "no data"
        series = spec.series[series_index % len(spec.series)]
        ...
        threshold = f" | thr {self._format_metric(spec.threshold)}" if spec.threshold is not None else ""
        return (
            f"latest {self._format_metric(latest)} | "
            f"Δ {delta_sign}{self._format_metric(delta)} | "
            f"avg {self._format_metric(avg)}{threshold}"
        )
